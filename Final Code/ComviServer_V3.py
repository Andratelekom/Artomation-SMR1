...
def pizza_incoming(circle):
    ...
    if (circle_x < 350 and circle_x > 305):
        return True
    else:
        return False
    ...
    def handle_client(self):
        while True:
            ...
            try:
                ...
                pizzas, dropping = Get_camera_info()
                print(f"Pizzas in crate: {pizzas}")
                benar = "3"
                salah = "4"
                self.conn.send(str(pizzas).encode())
                print(f"buffer: {dropping}")# Send the number to the client
                if dropping:
                    self.conn.send(str(benar).encode())
                    print(benar)
                elif dropping is False :
                    self.conn.send(str(salah).encode())
                    print(salah)
...
tcp_server = TCPServer('192.168.123.200', 20002)
tcp_server.start()
